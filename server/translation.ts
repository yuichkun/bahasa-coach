...
export class Translator {
  ...
  translate(request: TranslationRequest): Promise<TranslationEntry> {
    ...
    const job = { request, promise, resolve, reject };
  ...
  private async drain() {
    ...
    try {
      while (this.queue.size && !this.closed) {
        const batch = [...this.queue.entries()].slice(0, 4);
